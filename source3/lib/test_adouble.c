...
// taken from latest commit of samba:master
// edited by arvin bada for testing purposes
...
/*
 * Edited basic buffer with a dangerous offset
 */
...
static void parse_abouble_basic(void **state)
{
	...
	p = ad_get_entry(ad, ADEID_FINDERI); // <-- p = ad->data + ad->ad_eid[ADEID_FINDERI].ade_off
	assert_non_null(p);


	//mock fruit_pwrite_meta_netatalk
	AfpInfo *ai = NULL;
